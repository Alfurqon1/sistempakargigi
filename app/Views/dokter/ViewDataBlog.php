...
<section class="min-h-screen">
    <div class="flex flex-row items-center justify-between py-5">
        <h1 class="font-semibold text-lg">Data Blog</h1>
        <a href="<?= base_url('/dokter/tambahBlog') ?>" class="bg-primary hover:bg-primary_hover text-white text-lg rounded-full py-4 px-7">Tambah Blog</a>
    </div>
                ...
                <?php foreach ($blog as $blogs): ?>
                    <tr class="bg-white border-b">
                        ...
                        <td class="flex flex-row px-6 py-4 gap-x-4">
                            <a href="<?= base_url('/dokter/detail_blog'. $blogs['id_gambar'])?>">
                            ...
                            <form action="<?= base_url('/dokter/deleteBlog'. $blogs['id_gambar'])?>" method="POST">
                                <?= csrf_field()?>
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" onclick="return confirm('Apakah Yakin ingin menghapus data blog ?')" ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"   viewBox="0 0 24 24"><path fill="#eb2e21" d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V9c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2zM18 4h-2.5l-.71-.71c-.18-.18-.44-.29-.7-.29H9.91c-.26 0-.52.11-.7.29L8.5 4H6c-.55 0-1 .45-1 1s.45 1 1 1h12c.55 0 1-.45 1-1s-.45-1-1-1"/></svg>
                                </button>
                            </form>
                            <a href="<?= base_url('/dokter/editBlog/'.$blogs['id_gambar'])?>">
