   ...
   <section class="min-h-screen bg-cover bg-no-repeat bg-center flex justify-center items-center" style="background-image: url(<?= base_url()?>img/login_user.jpg);">
        <div class="text-center flex flex-col gap-y-4">
            ...
            <div class="rounded-3xl w-auto p-10 bg-white shadow-lg">
                ...
                <form action="<?= base_url('/forgotPassword') ?>" method="post" class="flex flex-col gap-y-6">
                 <?= csrf_field(); ?>
                    <div class="flex flex-col gap-y-5">
                        <input type="email" id="email" name="email" class="w-[500px] rounded-lg py-3 px-4 outline-none focus:ring-2 focus:ring-primary shadow-lg" placeholder="masukan email yg terdaftar">
                    </div>
                    
                    <div class="flex flex-col gap-y-5 font-bold">
                        <button class="w-[500px] bg-primary hover:bg-primary_hover text-white py-3 px-7 rounded-full text-center">Kirim Email</button>                    
                    </div>
